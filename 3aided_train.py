...
frozen_label=torch.randn(2,x_dim)#torch.load('model_data/label_reps/label_rep0.pt').requires_grad_(False)
# print(frozen_label)
node_features=torch.randn(graph.num_nodes()-2, x_dim)
graph.ndata['x'] = torch.cat((frozen_label,node_features),dim=0)
...
for epoch in range(EPOCH):
    ...
    node_rep=fake_node_features[label_idx_start:graph.num_nodes()]
...
import pickle
with open('model_data/LR.model','rb') as f:
    RL=pickle.load(f)
score=RL.score(node_features.numpy(),node_labels.numpy())
print(score)
exit()